...
>
> The two framing sentences above describe what SPECTRA is being built to be.
...
its gate exits zero on a clean clone with the network off. No gate below is implemented, so NO row
is green. Where code exists without its gate, the row says so rather than claiming completion.
...
core, components C1 through C10, none complete because no component gate exists), `ratchet.json` (test and gate
...
|------|-----------|---------|--------|
| A | Bash, Go, Python, Rust, SQL, TypeScript | The critical path. Never cut. | Python carries the reference slice; Rust and Go roots are empty; TypeScript and SQL not started |
...
This path exists because `make` is absent on the machine the slice was built on. It does not
replace the gated build above, and nothing run this way turns a milestone green.
