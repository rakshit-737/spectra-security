...
| [0016](0016-the-slice-implements-the-temporal-dispute-pass.md) | The slice implements the temporal-consistency pass, under the dispute protocol | Superseded in part by ADR-0017 | The slice-spec excluded it, so no ROBUST verdict could ever be blocked; 65.6 retains disputed licences so tampering cannot buy a stronger verdict. |
| [0017](0017-the-tamper-counterfactual-is-computed-in-the-prove-stage.md) | Correct ADR-0016: the tamper counterfactual belongs to the prove stage | Accepted | liveness.json is written and hashed before any verdict exists, so the counterfactual cannot live there. |
